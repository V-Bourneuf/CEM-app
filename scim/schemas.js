...
// SCIM 2.0 standard User schema (RFC 7643 §4.1) — only attributes our service handles
const USER_SCHEMA = {
    id:          URN.User,
    name:        'User',
...
// Okta CEM custom Role schema (per Okta's SCIM-with-Entitlements guide)
const ROLE_SCHEMA = {
    id:          URN.Role,
    name:        'Role',
    ...
    meta: { resourceType: 'Schema', location: `/scim/v2/Schemas/${URN.Role}` },
};

// Okta CEM custom Entitlement schema (per Okta's SCIM-with-Entitlements guide)
const ENTITLEMENT_SCHEMA = {
    ...
    meta: { resourceType: 'Schema', location: `/scim/v2/Schemas/${URN.Entitlement}` },
};
...
router.get('/ServiceProviderConfig', (req, res) => {
    res.json({
        ...
        authenticationSchemes: [{
            ...
            description: 'Static bearer token (SCIM_TOKEN env)',
            primary: true,
        }],
    });
});
...
// /Schemas — full list (User, Role, Entitlement)
router.get('/Schemas', (req, res) => {
    res.json(listResponse(ALL_SCHEMAS));
});

// /Schemas/{urn} — single schema by URN (Okta probes this during entitlement sync)
