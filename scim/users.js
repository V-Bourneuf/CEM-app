/**
 * SCIM /Users endpoints — full CRUD for Okta provisioning.
 *
...
function scimGrant(g) {
    return {
        value:   g.id,                       // catalog item ID — what Okta uses to reference it
...
function scimGrantsToIds(items) {
    ...
    for (const item of items) {
        ...
        if (!row) {
            // Fall back to value-column lookup, optionally type-scoped
...
router.get('/', (req, res) => {
    ...
    const { total, rows } = Users.list({ filter: req.query.filter, startIndex, count });
    res.json({
...
router.post('/', (req, res) => {
    ...
    const existing = Users.findByUserName(body.userName);
    if (existing) {
        return res.status(409).json(scimError(409, `User '${body.userName}' already exists`, 'uniqueness'));
    }

    const created = Users.create(pickFromBody(body));

    // Grants from initial create body — both roles and entitlements paths
    const grantIds = [
...
router.patch('/:id', (req, res) => {
    ...
    for (const op of ops) {
        ...
        // ----- entitlements OR roles (Okta uses both paths for CEM bindings) -----
        ...
        if (matchEnt || matchRole) {
            ...
            if (operation === 'add') {
                ...
                if (matchRole && matchEnt) {
                    // shouldn't happen — but if both, just set
                    Grants.setForUser(u.id, ids);
                } else if (matchRole) {
                    // replace only role grants
                ...
                if (ids.length === 0 && (path.includes('value eq') || path.includes('id eq'))) {
                    ...
                    // No specific value → remove all of this category
        ...
        // Bulk replace via {value: {field1: ..., field2: ...}}
